...
class SalesReturnController extends Controller
{
    ...
    public function findInvoice(Request $request)
    {
        ...
        if ($request->ajax()) {
            return Response()->json(['page' => view('backend.ajax.form.itemSelect', ['items' => $items])->render(),
                'item' => $item, 'project'=>$project, 'items'=>$items
    ...
    {
        $item=InvoiceItem::where('invoice_no',$request->invoice_no)->where('item_id',$request->item_name)->first();
        ...
        $itemReturn=SaleReturnTemp::where('invoice_no',$request->invoice_no)->where('item_id',$request->item_name)->first();
            ...
            {
            return Response()->json(['error' => 'Item quantity not more then sale quantity']);
            }
